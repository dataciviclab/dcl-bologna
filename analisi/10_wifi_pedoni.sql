...
-- Path: out/data/clean/bolognawifi_matrice/2025/bolognawifi_matrice_2025_clean.parquet
--
...
FROM read_parquet('out/data/clean/bolognawifi_matrice/2025/bolognawifi_matrice_2025_clean.parquet')
...
WITH flussi AS (
    SELECT id_origine as zona, totale FROM read_parquet('out/data/clean/bolognawifi_matrice/2025/bolognawifi_matrice_2025_clean.parquet') WHERE anno = 2025
    UNION ALL
    SELECT id_destinazione, totale FROM read_parquet('out/data/clean/bolognawifi_matrice/2025/bolognawifi_matrice_2025_clean.parquet') WHERE anno = 2025
)
SELECT max(label_zona) as zona, sum(totale) as flussi_totali
FROM flussi f JOIN (SELECT DISTINCT id_origine, max(label_origine) as label_zona FROM read_parquet('out/data/clean/bolognawifi_matrice/2025/bolognawifi_matrice_2025_clean.parquet') GROUP BY id_origine) z ON f.zona = z.id_origine
