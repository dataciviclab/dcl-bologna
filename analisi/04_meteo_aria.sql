...
SELECT 'temperature' as ds, min(date) as dal, max(date) as al FROM read_parquet('out/data/clean/temperature_bologna/2026/temperature_bologna_2026_clean.parquet')
UNION ALL
SELECT 'precipitazioni', min(date), max(date) FROM read_parquet('out/data/clean/precipitazioni_bologna/2026/precipitazioni_bologna_2026_clean.parquet')
UNION ALL
SELECT 'aria', min(reftime::date), max(reftime::date) FROM read_parquet('out/data/clean/centraline_aria/2026/centraline_aria_2026_clean.parquet');
...
SELECT stagione, 
       round(avg(pioggia_mm), 1) as pioggia_media_mm,
       count(*) as giorni
FROM read_parquet('out/data/clean/precipitazioni_bologna/2026/precipitazioni_bologna_2026_clean.parquet')
GROUP BY stagione ORDER BY stagione;
...
FROM read_parquet('out/data/clean/temperature_bologna/2026/temperature_bologna_2026_clean.parquet') t
JOIN read_parquet('out/data/clean/centraline_aria/2026/centraline_aria_2026_clean.parquet') a ON t.date = a.reftime::date
